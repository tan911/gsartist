...
const LocationForm: React.FC<LocationFormProps> = ({
    ...
    const [localCoordinates, setLocalCoordinates] = useState<[number, number] | null>(
        locationData?.coordinates
            ? [locationData.coordinates.lat, locationData.coordinates.lng]
            : null
    )

    const { isGeocoding, notification, handleMarkerDragEnd, handleGeocodeAddress } =
        useLocationGeocoding({ onLocationUpdate })

    // Update local coordinates when locationData changes
    useEffect(() => {
        if (locationData?.coordinates) {
            setLocalCoordinates([locationData.coordinates.lat, locationData.coordinates.lng])
        } else if (isEditing) {
