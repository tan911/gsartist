...
const MessageInput: React.FC<MessageInputProps> = ({
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const validImages = files.filter((file) => ACCEPTED_IMAGE_TYPES.includes(file.type))
    ...
    const handleSend = () => {
        if (selectedImages.length > 0 || selectedPdf) {
            onSend(selectedImages.length > 0 ? selectedImages : undefined, selectedPdf || undefined)
            setSelectedImages([])
            setSelectedPdf(null)
        } else {
            onSend()
    ...
    return (
        ...
        >
            ...
            <button
                ...
                disabled={
                    disabled || (!value.trim() && selectedImages.length === 0 && !selectedPdf)
                }
                type="button"
            >
