...
interface ChatAreaProps {
    ...
    onSendMessage: (conversationId: number, text: string, images?: File[], pdfFile?: File) => void
}

const ACCEPTED_IMAGE_TYPES = ['image/png', 'image/jpeg', 'image/jpg', 'image/webp']

const ChatArea: React.FC<ChatAreaProps> = ({ selectedConversation, onSendMessage }) => {
    ...
    const handleSendMessage = (images?: File[], pdfFile?: File) => {
        if (selectedConversation) {
            onSendMessage(selectedConversation.id, messageText.trim(), images, pdfFile)
            setMessageText('')
            setDroppedImages([])
            setDroppedPdf(null)
        }
    ...
    return (
        <div
            className={`flex-1 flex flex-col rounded-tr-lg rounded-br-lg relative ${
        ...
        >
            ...
            {/* Chat Header */}
            <ChatHeader client={selectedConversation.client} online={selectedConversation.online} />
            {/* Messages */}
