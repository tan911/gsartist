...
export const validateBookingSettings = (
  ...
  if (
    bookingSettings.maxAdvanceBooking &&
    bookingSettings.maxAdvanceBooking < 1
  ) {
 ...
 * @returns Summary string
 */
export const getBookingSettingsSummary = (
  settings: BookingSettings
): string => {
  const mode = settings.confirmationMode === "instant" ? "Instant" : "Manual";
  const buffer =
    settings.bufferTime > 0 ? `${settings.bufferTime}min buffer` : "No buffer";
