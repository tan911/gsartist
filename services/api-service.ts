...
class ApiService {
  ...
  async fetchAllUserData(
    ...
    try {
      ...
      const [
        ...
        this.api.get<ApiResponse<Conversation[]>>(
          `/users/${userId}/conversations`
        ),
        this.api.get<ApiResponse<PortfolioItem[]>>(
          `/users/${userId}/portfolio`
        ),
        this.api.get<ApiResponse<NotificationSettings>>(
          `/users/${userId}/notifications`
        ),
        this.api.get<ApiResponse<BookingSettings>>(
          `/users/${userId}/booking-settings`
        ),
        this.api.get<ApiResponse<LocationSettings>>(
          `/users/${userId}/location-settings`
        ),
        this.api.get<ApiResponse<VerificationData>>(
  ...
  async fetchUserDataSingleEndpoint(
    userId: string,
    forceRefresh = false
  ): Promise<UserData> {
    ...
    try {
      const response = await this.api.get<ApiResponse<UserData>>(
        `/users/${userId}/data`
      );
  ...
  private extractData<T>(
    ...
    if (
      ...
      promiseResult.value.data.success
    ) {
  ...
  async fetchUserProfile(userId: string): Promise<ProfileData> {
    const response = await this.api.get<ApiResponse<ProfileData>>(
  ...
  async fetchUserServices(userId: string): Promise<Service[]> {
    const response = await this.api.get<ApiResponse<Service[]>>(
